...
<head>
	...
	<script language="JavaScript" src="js/schedule3.js"></SCRIPT>
...
echo "<tr class=\"year\"><td colspan=\"2\"><br  />". $title . "<br  /><br  /></td></tr>";
...
while($num_rows0 > $in) {
	...
	while($rows0 = mysql_fetch_assoc($result0)) {
		...
		echo '<table border="1" class="floatyTable '.$strPartialName.'TopBar"><tr>';
			...
			foreach($ids as $id) {
				$sql = "SELECT * FROM foremen, employees WHERE foremen.employeeid = employees.idemployees AND foremen.employeeid = '$userID'";
				$result = mysql_query($sql);
				if(!$result) { $message  = 'Invalid query: ' . mysql_error() . "\n"; $message .= 'Whole query: ' . $result; die($message); }
		...
		echo '</tr></table>';
		echo '<table border="1" class="floatyTable '.$strPartialName.'TopBar"><tr><td style="background-color:'.$bgColor.'; width:125px;">'.$currentDepartmentName.'</td><td style="background-color:'.$bgColor.'; width:125px;">'.$currentDepartmentName.'</td></tr><tr><td style="background-color:'.$bgColor.'; width:125px;">BLOCK</td><td style="background-color:'.$bgColor.'; width:125px;">BLOCK</td></tr></table>';
	}
	...
	if($currentDepartment == 10){$classDeptName = "shop"; echo "<div id='menuItem'> <div id='" . $classDeptName . "Bar' class='colapsedMenu'>s<br />h<br />o<br />p<br />&nbsp;<br />g<br />u<br />y<br />s<br /></div> <div id='" . $classDeptName . "Content' class='floatL'>";}
		...
		while($days_in_month >= $day_num) {
			...
			while($cols <= $num_stake) {
				...
				echo "<td id=\"".$colname."\" style=\"width:125px\"><a class=\"iframe\" style=\"display:block;width:100%;\" href=\"editcol.php?editSelector=".$colname."&color=".$strdeptColor."\">".$colname."</a></td>"; 
				$cols++;
			}
			$colname = date('n') . "-".$day_num."-" . $strPartialName."-block-1";
			echo "<td id=\"" . $colname . "\" style=\"width:125px\"><a class=\"iframe\" style=\"display:block;width:100%;\" href=\"editcol.php?editSelector=".$colname."&color=".$strdeptColor."\">&nbsp;</a></td>"; 
			$cols++;
			$colname = date('n') . "-".$day_num."-" . $strPartialName."-block-2";
			echo "<td id=\"" . $colname . "\" style=\"width:125px\"><a class=\"iframe\" style=\"display:block;width:100%;\" href=\"editcol.php?editSelector=".$colname."&color=".$strdeptColor."\">&nbsp;</a></td>"; 
