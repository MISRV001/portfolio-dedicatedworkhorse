...
// set up a catch for the auto populate functionality
...
if(($editcolForeman!='block1')&&($editcolForeman!='block2')){
	$editcolYear = substr($editcolMaster, 12, 4);
} else {
						...
						<tr>
							...
							<td>
								<select id="editcolJob" style="width: 300px">
									...
									<?php
									//query for departments to be displayed first
									$sql = "SELECT idjobs, job_name FROM jobs WHERE progress <> 'declined' AND progress <> 'finished' OR progress IS NULL";	//awsarded or in progress
									$result = mysql_query($sql);
									if(!$result) {
										$message  = 'Invalid query: ' . mysql_error() . "\n";
										$message .= 'Whole query: ' . $result;
						...
						<tr>
							...
							<td>
								<select id="editcolVan" style="width: 300px">
									<?php
									// fetch the assigned van number
									$sql = "SELECT vannumber FROM van JOIN foremen_assign ON foremen_assign.vanid = van.vanid WHERE foremanid = '$foremanID' GROUP BY vannumber";
									$result = mysql_query($sql);
									if(!$result) {
										$message  = 'Invalid query: ' . mysql_error() . "\n";
										$message .= 'Whole query: ' . $result;
